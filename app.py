...
st.markdown("""
    <div style='background-color: #E8F5E9; padding: 20px; border-radius: 10px; margin: 20px 0;'>
        <h3 style='color: #2E7D32; margin: 0;'>Model Performance</h3>
        <p style='font-size: 24px; margin: 10px 0;'>Accuracy: {:.1f}%</p>
    </div>
...
# First column
with col1:
    st.markdown("<h3 style='color: #34495E;'>Acidity Measures</h3>", unsafe_allow_html=True)
...
# Second column
with col2:
    st.markdown("<h3 style='color: #34495E;'>Chemical Properties</h3>", unsafe_allow_html=True)
...
# Third column
with col3:
    st.markdown("<h3 style='color: #34495E;'>Other Properties</h3>", unsafe_allow_html=True)
...
if predict_button:
    ...
    color_map = {'Low': '#FF5252', 'Medium': '#FFA726', 'High': '#66BB6A'}
    bg_color_map = {'Low': '#FFEBEE', 'Medium': '#FFF3E0', 'High': '#E8F5E9'}
    
    st.markdown(f"""
        <div style='background-color: {bg_color_map[predicted_label]}; 
                    padding: 20px; border-radius: 10px; margin: 20px 0;'>
            <h3 style='color: {color_map[predicted_label]}; margin: 0;'>
        ...
        with prob_cols[idx]:
            st.markdown(f"""
                <div style='text-align: center; padding: 10px;'>
                    <h4 style='color: {color_map[label]};'>{label}</h4>
                    <p style='font-size: 20px;'>{prob * 100:.1f}%</p>
                </div>
            """, unsafe_allow_html=True)

# Footer with information
st.markdown("""
    <div style='background-color: #F8F9FA; padding: 20px; border-radius: 10px; margin-top: 50px;'>
        <h3 style='color: #2C3E50;'>About the Model</h3>
        <p>This wine quality prediction system uses a K-Nearest Neighbors (KNN) model trained on a dataset of red wine samples. 
        The model considers various chemical properties to predict wine quality as Low, Medium, or High.</p>
        <p>The model uses distance-weighted KNN with k=5 neighbors for prediction.</p>
    </div>
""", unsafe_allow_html=True)